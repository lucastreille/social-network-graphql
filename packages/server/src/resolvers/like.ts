...
const likeQueries: Pick<LikeResolvers<Context>["Query"], "like" | "likes"> = {
  like: async (_parent: EmptyObject, args: QueryLikeArgs, { prisma }: Context) => {
  ...
  likes: async (_parent: EmptyObject, _args: EmptyObject, { prisma }: Context) => {
...
const likeMutations: Pick<LikeResolvers<Context>["Mutation"], "addLike"> = {
  addLike: async (_parent: EmptyObject, { postId }: MutationAddLikeArgs, { prisma, user }: Context): Promise<string> => {
    if (!user) {
